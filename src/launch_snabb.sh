...
sudo snabb lwaftr run --conf ${INT}.conf --on-a-stick $PCI
EOF
  cp ${INT}.conf test-snabbvmx-lwaftr-${INT}.conf
  sed -i "s/cache_refresh_interval.*/cache_refresh_interval = 0,\n    next_hop_mac = \"02:02:02:02:02:02\",/" test-snabbvmx-lwaftr-${INT}.conf
  cat > test_snabb_snabbvmx_${INT}.sh <<EOF
#!/bin/bash
echo "launching snabb snabbvmx on-a-stick on PCI $PCI"
sudo nabb snabbvmx lwaftr --conf test-snabbvmx-lwaftr-${INT}.conf --id $INT --pci $PCI --mac $MAC
EOF
  chmod a+rx test_snabb_*sh

  echo "launch snabbvmx for $INT on cpu $CORE (node $NODE) after $SLEEP seconds ..."
  CMD="$NUMACTL snabb snabbvmx lwaftr --conf ${INT}.conf --id $INT --pci $PCI --mac $MAC --sock ext$INT --mirror tap${INT:2:1}"
  echo $CMD
  sleep $SLEEP
