...
def main():
    ...
    while True:
        ...
        if debug:
            print("testing nexthops: ", new_nexthops.keys())

        # Create a MultiPing object to test three hosts / addresses
        mp = multiping.MultiPing(new_nexthops.keys())

        # Send the pings to those addresses
        mp.send()

        # wait at most 1 sec for responses (may return sooner if all results
        # are received)
        reachable_nexthops, no_responses = mp.receive(1)
        reachable_nexthops_keys = sorted(reachable_nexthops.keys())
        ...
        print("received %d routes with %d (out of %d) reachable next hops" %
              (len(new_routes), len(reachable_nexthops), len(new_nexthops)))
        if debug:
